 ...
 */
export const generateGlitchBits = (grid: GridDimensions, count: number): GlitchBit[] => {
 ...
 */
export const updateGlitchBits = (bits: GlitchBit[], deltaTime: number): GlitchBit[] => {
  return bits
    .map((bit) => ({
      ...bit,
      life: bit.life + deltaTime,
      opacity:
        bit.life < bit.maxLife ? Math.sin((bit.life / bit.maxLife) * Math.PI) * 0.8 + 0.2 : 0,
    }))
...
export const getCurrentPhase = (elapsedTime: number, phases: PhaseConfig[]): PhaseConfig => {
  return (
    phases.find((phase) => elapsedTime >= phase.startTime && elapsedTime < phase.endTime) ??
    phases[phases.length - 1]!
  );
};

export const getPhaseProgress = (elapsedTime: number, phase: PhaseConfig): number => {
