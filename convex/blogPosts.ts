...
/**
 * Get post content by ID (for admin editor)
 * Fetches from blogPostContent table, falls back to deprecated blogPosts.content
 */
export const getContent = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!post) return null;
    return await getPostContent(ctx, args.postId, post.content);
  },
});
