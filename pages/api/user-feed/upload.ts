...
// File type configurations (image and video only for user feed)
const FILE_CONFIGS = {
  image: {
    maxSize: 10 * 1024 * 1024, // 10MB
    allowedTypes: ["image/jpeg", "image/png", "image/webp", "image/gif"],
    folder: "user-feed/images",
  },
  video: {
    maxSize: 25 * 1024 * 1024, // 25MB
    allowedTypes: ["video/mp4", "video/webm", "video/quicktime"],
    folder: "user-feed/videos",
  },
} as const;
  ...
  try {
    ...
    if (!category) {
      return res.status(400).json({
        error: `Unsupported file type: ${mimeType}. Allowed: images (JPEG, PNG, WebP, GIF), videos (MP4, WebM)`,
      });
    }
    ...
    console.error("User feed upload error:", error);
