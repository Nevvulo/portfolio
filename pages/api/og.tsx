...
// Skull SVG component reused across templates
...
// Branding row (skull + "nevulo" text)
function Branding({ skullSize, fontSize }: { skullSize: number; fontSize: number }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
      <SkullSvg size={skullSize} />
      <span
        style={{
...
export default async function handler(req: NextRequest) {
  try {
    ...
    const fonts = [
      { name: "Sixtyfour", data: sixtyfourFont, style: "normal" as const, weight: 400 as const },
      { name: "Inter", data: interFont, style: "normal" as const, weight: 700 as const },
    ];

    // Article template: faded background image with branded overlay
    if (type === "article" && image) {
      return new ImageResponse(
        ...
        >
          ...
          >
            <Branding skullSize={64} fontSize={32} />
    ...
    // Default template with title/subtitle: branding top-left, text below
    if (title) {
      return new ImageResponse(
        <div
          style={{
