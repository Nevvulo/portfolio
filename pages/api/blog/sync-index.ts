...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Fetch all published public posts from Postgres
    console.log("[sync] Fetching posts from Postgres...");
    ...
    if (!posts || posts.length === 0) {
      return res.status(200).json({
        success: false,
        synced: 0,
        message: "No posts found from Postgres",
        debug: { postsCount: 0 },
      });
    }
    ...
    if (documents.length === 0) {
      return res.status(200).json({
        ...
        debug: {
          ...
          publicPosts: 0,
          postVisibilities: posts.map((p) => ({ slug: p.slug, visibility: p.visibility })),
        },
      });
    }
