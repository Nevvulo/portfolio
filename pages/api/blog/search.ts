...
import { blogIndex, type BlogPostContent, type BlogPostMetadata } from "../../../lib/upstash-search";
...
export default async function handler(
	...
	try {
		const labelArray = labels
			? (typeof labels === "string" ? labels.split(",") : labels).map((l) => l.trim()).filter(Boolean)
			: undefined;
