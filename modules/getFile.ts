import type { Blogmap, BlogPostContent } from "../types/blog";
...
> {
  try {
    if (isKnownFileLocation(location)) {
      const res = await fetch(`https://raw.githubusercontent.com/Nevvulo/blog/main/${location}`);
...
const isBlogmap = (location: Location): location is "blogmap.json" => location === "blogmap.json";
const isBlogPost = (location: Location): location is `posts/${string}` =>
  location.startsWith("posts/");
const isSchema = (location: Location): location is "schema.json" => location === "schema.json";
